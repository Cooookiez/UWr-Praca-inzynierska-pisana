...
\newline
System oparty jest na Raspberry Pi 4B, na języku programowania python 3\textcolor{red}{, oraz} wykorzystuje elementy wydrukowane na drukarce 3D Creality Ender-3 v.2
\newline
...
\begin{enumerate} % itemize
  ...
  \begin{itemize}
  	...
  	\item micro-komputera Rasppery Pi, odpowiedzialnej za przetworzenie sygnału oraz z kamerki, oraz sprawdzenie czy znamy tą osobe i wyświetlenia oraz wypowiedzenia odpowidniego komunikatu
  	\item głośnika, stanowiącego element wyjściowy z systemu i służącego do podawania komunikatów głosowych. Głośnij jest podpinany pod port jack 3,5mm, więc jest dowolność w wyborze.
  	\item \textcolor{red}{Wyświetlacza, stanowiący część wizualną powitania. Wyświetlacz jet podłączony pinami do Raspberry Pi}
  \end{itemize}
  \item Warstwę programistyczną odpowiedzialną za:
  \begin{itemize}
  	\item obróbkę oraz optymalizację odczytanego obrazu
  	\item interpretację pobranego obrazu oraz porównanie go z wcześniej zdefiniowaną bazą zdjęć (biblioteka) \textcolor{red}{– Teraz jak czytam to ta biblikoteka sugeruje mie że zdjęcia to biblioteka, a nie to coś co używam w kodzie}
...
\newline
The system is based on a Raspberry Pi 4B, the python 3 programming language, and uses components printed on a Creality Ender-3 v.2 3D printer
\newline
...
\begin{enumerate} % itemize
  ...
  \begin{itemize}
  	...
  	\item the Rasppery Pi microcomputer, which is responsible for processing the camera signal and checking whether we know the person and displaying and pronouncing the appropriate message
  	\item A speaker, which is the output from the system and is used to give voice messages. The speaker is plugged into a 3.5mm jack port, so there is flexibility in choice.
  	\item Display, which forms the visual part of the greeting. The display is pinned to the Raspberry Pi
  \end{itemize}
...
\newline
Projekt z jednej strony pokazuje, że połączenie wiedzy z zakresu informatyki oraz projektowania dostarcza każdemu studentowi odpowiednią wiedzę oraz umiejętności, do wykonania prototypu od fazy projektowej do fazy praktycznego zastosowania,  z drugiej udowadnia, że wiedza ta pozwala na wykonania projektu z wykorzystaniem elementów ogólnodostępnych na rynku.
...
\subsection{Płyta Raspberry Pi}\textsc{
